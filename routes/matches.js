...
router.get("/", function (req, res, next) {
  calls.getMatches().then((matches) => {
		calls.getSeasons().then((seasons) => {
			...
			calls.getMatchesForSeason(curMatch.season.id).then((seasonMatches) => {
							...
							calls.getInducementsForMatchAndTeam(curMatch.id, curMatch.team2.id).then((team2Inducements) => {
								calls.getPurchasesForMatchAndTeam(curMatch.id, curMatch.team1.id).then((team1Purchases) => {
									calls.getPurchasesForMatchAndTeam(curMatch.id, curMatch.team2.id).then((team2Purchases) => {
										...
										curMatch.number = seasonMatches.findIndex(match => match.id == curMatch.id) + 1
										
										res.render("matches", { "seasons": seasons, "seasonMatches": seasonMatches, "curMatch": curMatch, "team1MatchPlayers": team1MatchPlayers, "team2MatchPlayers": team2MatchPlayers, "team1Inducements": team1Inducements, "team2Inducements": team2Inducements, "team1Purchases": team1Purchases, "team2Purchases": team2Purchases })
									})
								})
							})
						})
